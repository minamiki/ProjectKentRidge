<!-- This page is similar to recentActivity.php but there is additional paging and there is no header "Recent Activity".
-display the recent activity for both user and friends
-also including the user's current level and achievement
-display pages by dividing the total number of recent activities by number of records allowed in 1 page
-->
<?php
require('../modules/quizrooDB.php'); // database connection
require('../modules/variables.php');
require('../modules/member.php'); // require operations on members

$member = new Member(); // create new member
...
/****************************************************************************************************************
 * Add results of users to recent activities array: loop through the query result and add to the recent activity array
 ****************************************************************************************************************/
...
/****************************************************************************************************************
 * Add achievements of users to recent activities array: loop through the query result and add to the recent activity query
 ****************************************************************************************************************/
...
// divide total recent activities by maximum number of activities allowed in 1 page
...
<?php 
$activity_count = 0;
// if recent activity count >0, add a division to display the recent activity 
...
<?php }
    ...
    if($activity['type']==0){	
        ...
        ?>			
        <!-- Display the recent activity on Quizroo-->
        ...
        <?php }else{?>
        <!-- Display what the user got in the latest quiz he took -->
    ...
    <?php }else if($activity['type']==1){
        if($activity['information']['member_id']){ ?>
        <!-- Display the user's achievement-->
        ...
        <?php }else{?>
            ...
            <?php } else {?>
            <!-- Display the achievement of user-->
  ...
  <?php }?>
<?php } ?>
<!-- This division if for displaying the paging-->
<div id="paging">
  <table border="0" align="center" cellpadding="5" cellspacing="0">
    ...
    <!-- for first page-->
      ...
      <td><?php if ($first_entry > 0) { // Show if not first page ?>
        ...
        <?php } // Show if not first page ?></td>
      <td>&nbsp;</td>
      <td><?php // calculate the page range displayed according to total number of pages and the position of current page
